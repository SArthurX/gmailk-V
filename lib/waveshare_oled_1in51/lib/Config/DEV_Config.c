...
#elif USE_WIRINGX_LIB
    Read_value = (digitalRead(Pin) == HIGH) ? 1 : 0;
#endif
    return Read_value;
}
...
UBYTE DEV_ModuleInit(void)
{
...
#elif USE_WIRINGX_LIB
    if (wiringXSetup(WIRINGX_PLATFORM, NULL) == -1) {
        printf("wiringX init failed !!! \r\n");
        wiringXGC();
        return 1;
    } else {
        printf("wiringX init success !!! \r\n");
    }

    DEV_GPIO_Init();
#if USE_SPI
    printf("USE_SPI\r\n");
    SPI_Handle = wiringXSPISetup(SPI_CHANNEL, SPI_SPEED_HZ);
    if (SPI_Handle < 0) {
        printf("wiringX SPI setup failed !!!\r\n");
        return 1;
    }

    int spi_fd = wiringXSPIGetFd(SPI_CHANNEL);
    if (spi_fd >= 0) {
        uint8_t mode = (uint8_t)(SPI_MODE_3 | (SPI_USE_HW_CS ? 0 : SPI_NO_CS));
        uint8_t bits = 8;
    ...
    OLED_DC_0;
    OLED_CS_0;
    I2C_Handle = wiringXI2CSetup("/dev/i2c-0", 0x3c);
    if (I2C_Handle < 0) {
        printf("wiringX I2C setup failed !!!\r\n");
        return 1;
    }
#endif
#endif
...
#elif USE_WIRINGX_LIB
    wiringXSPIDataRW(SPI_CHANNEL, pData, (int)Len);
...
#elif USE_WIRINGX_LIB
    if (I2C_Handle >= 0)
        wiringXI2CWriteReg8(I2C_Handle, Cmd, value);
